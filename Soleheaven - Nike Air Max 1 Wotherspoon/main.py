...
def enter(size, domain, proxy_list, entry, entries):
    s = requests.session()
    try:
        r = s.get("https://www.soleheaven.com/pages/wotherspoon-raffle", timeout=5, verify=False)
    except:
    ...
    try:
        r = s.post(link, headers=headers, cookies=cookies, data=payload, proxies=proxies, timeout=5, verify=False)
    except:
